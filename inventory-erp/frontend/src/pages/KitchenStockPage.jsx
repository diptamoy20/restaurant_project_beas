...
export function KitchenStockPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      setLoading(true);
      const res = await apiFetch(`stock-movement/kitchen/stock?restaurantId=${restaurantId}`);
      setStock(res);
      setError(null);
    } catch (err) {
      setError(err.message || 'Failed to load kitchen inventory.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && stock.length === 0) return <Loader label="Loading kitchen operational inventory..." />;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Chef station operational stock. Auto-deducted when orders move to PREPARING status.</p>
        </div>
      </div>

      <Card title="Kitchen Stock Balances" eyebrow={restaurantId ? `Restaurant ID: ${restaurantId}` : 'Select a restaurant workspace'}>
        <Table columns={columns} data={stock} emptyMessage="No kitchen inventory records. Transfer stock from the store room." />
      </Card>
    </div>
  );
}
