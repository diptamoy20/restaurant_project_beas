...
export function StoreStockPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await apiFetch(`operations/${slug}/store-inventory`);
  ...
  if (loading) return <Loader label="Loading store inventory..." />;
  ...
  const columns = [
    ...
    { key: 'reservedQuantity', header: 'Reserved', render: (row) => (
      <span className="text-slate-600">{row.reservedQuantity ?? 0}</span>
    )},
    ...
    { key: 'maximumStock', header: 'Max Stock', render: (row) => row.maximumStock ?? 0 },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Restaurant dry store room stock levels</p>
      </div>

      <Card>
        {items.length === 0 ? (
          <EmptyState message="No store items found." />
        ) : (
          <Table columns={columns} data={items} emptyMessage="No store items found." />
        )}
      </Card>
    </div>
  );
