...
export function SelectedRestaurantProvider({ children }) {
  ...
  const [
    ...
    setSelectedRestaurantSlugState,
  ] = useState(() => {
    if (typeof window === 'undefined') {
      return null;
    }
    ...
    ({ id, slug }) => {
      ...
      if (id == null) {
        window.sessionStorage.removeItem(
          'restaurant-web-active-restaurant',
        );
      } else {
      ...
      if (!slug) {
        window.sessionStorage.removeItem(
          'restaurant-web-active-restaurant-slug',
        );
      } else {
  ...
  const clearSelectedRestaurant = useCallback(() => {
    ...
    setSelectedRestaurantSlugState(null);
  ...
  useEffect(() => {
    ...
    if (
      slugFromPath &&
      slugFromPath !== selectedRestaurantSlug
    ) {
      setSelectedRestaurantSlugState(
        slugFromPath,
      );
