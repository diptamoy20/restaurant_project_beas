...
import { useGetKitchenInventoryQuery, useGetTransactionLedgerQuery } from '../../services/inventoryApi';
...
export function KitchenInventoryPage() {
  ...
  const { data: transactions } = useGetTransactionLedgerQuery(
    selectedItem ? { itemId: selectedItem.itemId, inventoryType: 'KITCHEN' } : undefined,
    { skip: !selectedItem }
  );
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Chef's Operational stock room. Consumed automatically upon customer orders status changes to PREPARING.
        </p>
      </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 dark:bg-slate-800/40 border-b border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-400 uppercase">
                <tr>
                  ...
                  <th className="py-3 px-4 text-right">Kitchen Available Qty</th>
                  <th className="py-3 px-4 text-right">Kitchen Min Stock</th>
          ...
          <div className="w-full max-w-3xl rounded-3xl bg-white dark:bg-slate-900 p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Kitchen stock movements & recipe deductions</p>
              </div>
              <button
