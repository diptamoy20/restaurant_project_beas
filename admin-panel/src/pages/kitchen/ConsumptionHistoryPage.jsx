...
import { useGetConsumptionHistoryQuery } from '../../services/kitchenApi';
...
export function ConsumptionHistoryPage() {
  ...
  const { data: history = [], isLoading, error } = useGetConsumptionHistoryQuery(restaurantId, { skip: !restaurantId });
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Read-only audit log of ingredient deductions triggered automatically when orders enter PREPARING status.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 text-center text-slate-500">
          No ingredient consumption logged yet. Deductions occur automatically when orders are set to PREPARING.
        </div>
      ) : (
